...
class QuantizedModel(nn.Module):
    ...
    def feature(self, inputs):
        embedding = self.model.embeddings(input_ids=inputs['input_ids'])
        embedding = self.quant(embedding)

        outputs = self.model(attention_mask=inputs['attention_mask'], inputs_embeds=embedding)
        if self.cfg.weighted_layer_pooling:
            feature = self.pool(outputs['hidden_states'])
            feature = self.mean_pool(feature, inputs['attention_mask'])
            # feature = features['token_embeddings'][:, 0]
