...
from utils import get_score, get_args, convert_dot_dict, seed_everything
...
def main():
    ...
    if isinstance(CFG.teacher_model, list):
        train_df_ = ensemble(CFG.teacher_model, verbose=True)
    else:
        train_df_ = pd.read_csv(os.path.join(CFG.teacher_model, 'oof_df.csv'))
    train_df = train_df.merge(train_df_[['text_id'] + [f'pred_{c}' for c in CFG.target_cols]], on='text_id', how='left')
